...
namespace TSL.FiniteStateMachine {
  ...
  public abstract class StateDriver {
    ...
    public abstract void StartMachine(FSM fsm);
    ...
    public abstract bool IsInState(FSM fsm);
    ...
    public abstract void ForceStateChangeOn(FSM fsm);
    ...
    public abstract void SampleClip(FSM fsm, float time);
    #endif   
  }
  ...
  public class StateDriver<S> : StateDriver where S : State, new() {
    ...
    /// </summary>
    /// <param name="fsm">The machine to start.</param>
    public override void StartMachine(FSM fsm) {
      if (!fsm.Running) {
    ...
    /// <param name="fsm">The state machine to check this for.</param>
    /// <returns>True if the state machine is already in the state.</returns>
    public override bool IsInState(FSM fsm) => fsm.IsInState<S>();

    /// <summary>
    /// Forcibly change the state of the machine.
    /// </summary>
    /// <param name="fsm">The machine to force a state change on.</param>
    public override void ForceStateChangeOn(FSM fsm) {
      #if UNITY_EDITOR
    ...
    /// <param name="fsm">The machine to sample this clip on.</param>
    /// <param name="time">The timestamp of the clip to sample</param>
    public override void SampleClip(FSM fsm, float time) {
      if (cachedClip == null) {
    ...
    private AnimationClip FindClip(FSM fsm) {
